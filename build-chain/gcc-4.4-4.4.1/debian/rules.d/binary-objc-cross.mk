...
ifeq ($(with_lib64objc),yes)
	mkdir -p $(d_objc)/$(gcc_lib_dir)/$(biarch64subdir)
	mv $(d)/$(PF)/$(DEB_TARGET_GNU_TYPE)/lib64/libobjc*.{a,la} $(d_objc)/$(gcc_lib_dir)/$(biarch64subdir)/
	dh_link -p$(p_objc) \
	  /$(PF)/$(DEB_TARGET_GNU_TYPE)/lib64/libobjc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarch64subdir)/libobjc.so
endif
ifeq ($(with_lib32objc),yes)
	mkdir -p $(d_objc)/$(gcc_lib_dir)/$(biarch32subdir)
	mv $(d)/$(DEB_TARGET_GNU_TYPE)/$(lib32)/libobjc*.{a,la} $(d_objc)/$(gcc_lib_dir)/$(biarch32subdir)/
	dh_link -p$(p_objc) \
	  /$(lib32)/$(DEB_TARGET_GNU_TYPE)/libobjc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarch32subdir)/libobjc.so
endif
ifeq ($(with_libn32objc),yes)
	mkdir -p $(d_objc)/$(gcc_lib_dir)/$(biarchn32subdir)
	mv $(d)/$(DEB_TARGET_GNU_TYPE)/$(libn32)/libobjc*.{a,la} $(d_objc)/$(gcc_lib_dir)/$(biarchn32subdir)/
	dh_link -p$(p_objc) \
	  /$(libn32)/$(DEB_TARGET_GNU_TYPE)/libobjc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarchn32subdir)/libobjc.so
endif
