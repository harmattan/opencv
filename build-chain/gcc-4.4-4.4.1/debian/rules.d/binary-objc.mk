...
ifeq ($(biarch64),yes)
	...
	dh_link -p$(p_objc_m) \
	  /$(PF)/lib64/libobjc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarch64subdir)/libobjc.so
  ifeq ($(with_objc_gc),yes)
	dh_link -p$(p_objc_m) \
	  /$(PF)/lib64/libobjc_gc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarch64subdir)/libobjc_gc.so
  endif
endif
ifeq ($(biarch32),yes)
	rm -f $(d)/$(lib32)/libobjc*.{la,so}
	mkdir -p $(d_objc_m)/$(gcc_lib_dir)/$(biarch32subdir)
	mv $(d)/$(lib32)/libobjc*.a $(d_objc_m)/$(gcc_lib_dir)/$(biarch32subdir)/
	dh_link -p$(p_objc_m) \
	  /$(lib32)/libobjc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarch32subdir)/libobjc.so
  ifeq ($(with_objc_gc),yes)
	dh_link -p$(p_objc_m) \
	  /$(lib32)/libobjc_gc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarch32subdir)/libobjc_gc.so
  endif
endif
ifeq ($(biarchn32),yes)
	rm -f $(d)/$(PF)/$(libn32)/libobjc*.{la,so}
	mkdir -p $(d_objc_m)/$(gcc_lib_dir)/$(biarchn32subdir)
	mv $(d)/$(PF)/$(libn32)/libobjc*.a $(d_objc_m)/$(gcc_lib_dir)/$(biarchn32subdir)/
	dh_link -p$(p_objc_m) \
	  /$(libn32)/libobjc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarchn32subdir)/libobjc.so
  ifeq ($(with_objc_gc),yes)
	dh_link -p$(p_objc_m) \
	  /$(libn32)/libobjc_gc.so.$(OBJC_SONAME) /$(gcc_lib_dir)/$(biarchn32subdir)/libobjc_gc.so
  endif
endif
