...
#	mv $(d)/$(gcc_lib_dir)/libobjc_gc.so.* $(d)/$(PF)/$(libdir)/.
	...
	dh_makeshlibs -p$(p_lobjc) -Xlibobjc_gc.so -V '$(p_lobjc) (>= $(DEB_SOVERSION))'
...
ifeq ($(with_objc_gc),yes)
	...
	dh_makeshlibs -p$(p_l64objc) -Xlibobjc_gc.so -V '$(p_l64objc) (>= $(DEB_SOVERSION))'
...
ifeq ($(with_objc_gc),yes)
	...
	dh_makeshlibs -p$(p_l32objc) -Xlibobjc_gc.so -V '$(p_l32objc) (>= $(DEB_SOVERSION))'
...
$(binary_stamp)-libn32objc: $(install_stamp)
	...
	rm -rf $(d_ln32objc) $(d_ln32objcdbg)
	dh_installdirs -p$(p_ln32objc) \
		$(PF)/$(libn32)
	DH_COMPAT=2 dh_movefiles -p$(p_ln32objc) \
		$(PF)/$(libn32)/libobjc.so.*
ifeq ($(with_objc_gc),yes)
	DH_COMPAT=2 dh_movefiles -p$(p_ln32objc) \
		$(PF)/$(libn32)/libobjc_gc.so.*
endif
	debian/dh_doclink -p$(p_ln32objc) $(p_base)
	debian/dh_doclink -p$(p_ln32objcdbg) $(p_base)

	dh_strip -p$(p_ln32objc) --dbg-package=$(p_ln32objcdbg)
	dh_compress -p$(p_ln32objc) -p$(p_ln32objcdbg)
	dh_fixperms -p$(p_ln32objc) -p$(p_ln32objcdbg)
	dh_makeshlibs -p$(p_ln32objc) -V '$(p_ln32objc) (>= $(DEB_SOVERSION))'
#	dh_shlibdeps -p$(p_ln32objc)
	dh_gencontrol -p$(p_ln32objc) -p$(p_ln32objcdbg) \
		-- -v$(DEB_VERSION) $(common_substvars)
	dh_installdeb -p$(p_ln32objc) -p$(p_ln32objcdbg)
	dh_md5sums -p$(p_ln32objc) -p$(p_ln32objcdbg)
	dh_builddeb -p$(p_ln32objc) -p$(p_ln32objcdbg)
